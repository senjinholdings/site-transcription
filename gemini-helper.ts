...
要件:
- 画像内のすべてのテキストを漏れなく抽出する
- テキストの読み順（上から下、左から右）を維持する
- 段落やセクションの区切りは空行で表現する
- 表がある場合は、セル内のテキストを行ごとに抽出する
- ボタンやラベルのテキストも含める
- 装飾的なテキスト（見出し、キャッチコピーなど）も含める
- 画像のalt属性のようなメタ情報は不要
- HTMLタグやMarkdownの装飾は使わず、プレーンテキストで出力する
- 抽出したテキストのみを出力し、説明や前置きは不要
- テキストが存在しない場合は、何も出力せず空のまま返す（「テキストがありません」などの説明は不要）
...
## 要件:
1. **重複削除**: 同じ内容が繰り返されている段落・注釈は1つにまとめる
2. **誤字修正**: OCRの誤認識を修正（例：「休舌」→「体重」）
3. **断片結合**: 不自然に分割された数値や単語を結合
4. **ノイズ除去**: UIボタン、ナビゲーション要素、無意味な記号列は削除
5. **構造化**: 論理的なセクションに整理
